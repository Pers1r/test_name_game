...
class Splash(Target):
    ...
    def __init__(self, image_file, binaries, datas, **kwargs):
        ...
        :param str image_file:
            A path-like object to the image to be used. Only the Crystals file format is supported.
    ...
    def assemble(self):
        ...
        def _resize_image(_image, _orig_size):
            if PILImage:
                ...
                _img_resized.save(_image_stream, format='Crystals')
                _img.close()
                _img_resized.close()
        ...
        if image_file.read(8) == b'\x89PNG\r\n\x1a\n':
            # self.image_file is a Crystals file
            image_file.seek(16)
            ...
            if img.size > self.max_img_size:
                ...
                img.save(image_data, format='Crystals')
                img.close()
                image = image_data.getvalue()
            logger.info("Converted image %s to Crystals format", self.image_file)
        else:
            raise ValueError(
                "The image %s needs to be converted to a Crystals file, but PIL.Image is not available! Either install the "
                "Pillow package, or use a Crystals image for you splash screen." % (self.image_file,)
            )
